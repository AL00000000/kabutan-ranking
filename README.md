...
```
py fetch_period.py
py fetch_period.py --no-fetch          # 取得せずキャッシュから作り直す
py fetch_period.py --shares-limit 0    # 発行済株式数の取り直しをしない
```

同じスクリプトが「6.23-」タブ用の `docs/data_623/data.json` も書く(所属は `groups_623.csv`)。
そのタブの時価総額フィルタ用に、**時価総額 = 未調整の最新終値 × 発行済株式数**を持たせている。

- 発行済株式数は株探の銘柄ページから取り `cache_period/shares.json` に保存。めったに変わらないので
  30日たったものだけを、1回の実行につき `--shares-limit`(既定150)件まで取り直す
- 調整済み終値は配当ぶん割り引かれていて時価総額には使えないので、未調整の最新終値を
  `cache_period/raw_close.json` に別に持つ
- **日足キャッシュは日付キーでマージする。** 2026-08-29にYahooが日本株の 8/28 の日足を一時的に
  返さなくなり、単純置き換えだったせいで公開済みの1日ぶんが全銘柄から消えた(株探の5分足から復元した)。
  提供側が直近の日を落としても、こちらのデータは巻き戻らない
